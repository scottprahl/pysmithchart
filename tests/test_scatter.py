...
class TestSmithAxesScatter:
    ...
    def test_scatter_returns_path_collection(self, smith_axes):
        ...
        collection = smith_axes.scatter([50 + 25j], domain=Z_DOMAIN)
    ...
    def test_scatter_complex_input_REFLECTION_DOMAIN(self, smith_axes):
        ...
        collection = smith_axes.scatter(ZL, s=100, domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_separate_xy_REFLECTION_DOMAIN(self, smith_axes):
        ...
        x_vals = [30, 50, 100]
        y_vals = [30, 50, 100]
        collection = smith_axes.scatter(x_vals, y_vals, s=100, domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_single_point(self, smith_axes):
        ...
        collection = smith_axes.scatter(50 + 25j, s=150, domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_REFLECTION_DOMAIN2(self, smith_axes):
        ...
        collection = smith_axes.scatter(S, s=100, c="blue", domain=R_DOMAIN)
        assert isinstance(collection, PathCollection)
        assert len(collection.get_offsets()) == len(S)

    def test_scatter_Y_DOMAIN(self, smith_axes):
        """Test scatter with Y-parameters (admittance)."""
        Y = [0.02 + 0.01j, 0.01 - 0.01j]
        collection = smith_axes.scatter(Y, s=100, domain=Y_DOMAIN)
        assert isinstance(collection, PathCollection)
        assert len(collection.get_offsets()) == len(Y)

    def test_scatter_NORM_Z_DOMAIN(self, smith_axes):
        """Test scatter with A-parameters (absolute coordinates)."""
        A = [1.0 + 0.5j, 0.5 + 0.3j]
        collection = smith_axes.scatter(A, s=100, domain=NORM_Z_DOMAIN)
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_with_color(self, smith_axes):
        ...
        collection = smith_axes.scatter([50 + 25j, 75 + 50j], s=100, c="red", domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_with_colormap(self, smith_axes):
        ...
        colors = [0, 0.5, 1.0]
        collection = smith_axes.scatter(points, s=100, c=colors, cmap="viridis", domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_with_varying_sizes(self, smith_axes):
        ...
        points = [30 + 30j, 50 + 50j, 100 + 100j]
        sizes = [50, 100, 150]
        collection = smith_axes.scatter(points, s=sizes, domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)

    def test_scatter_with_alpha(self, smith_axes):
        """Test scatter with alpha transparency."""
        collection = smith_axes.scatter([50 + 25j], s=150, alpha=0.5, domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)

    def test_scatter_with_edgecolors(self, smith_axes):
        """Test scatter with edge colors."""
        collection = smith_axes.scatter(
            [50 + 25j, 75 + 50j], s=100, c="blue", edgecolors="red", linewidths=2, domain=Z_DOMAIN
        )
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_with_marker_style(self, smith_axes):
        ...
        for i, marker in enumerate(markers):
            collection = smith_axes.scatter([50 + i * 10j], s=100, marker=marker, domain=Z_DOMAIN)
            assert isinstance(collection, PathCollection)
    ...
    def test_scatter_REFLECTION_DOMAIN2_warning_outside_chart(self, smith_axes):
        ...
        with pytest.warns(UserWarning, match="S-parameter magnitude"):
            smith_axes.scatter([1.5 + 0j], domain=R_DOMAIN)

    def test_scatter_multiple_calls(self, smith_axes):
        """Test multiple scatter calls on same axes."""
        c1 = smith_axes.scatter([30 + 30j], s=100, c="red", domain=Z_DOMAIN)
        c2 = smith_axes.scatter([50 + 50j], s=100, c="blue", domain=Z_DOMAIN)
        c3 = smith_axes.scatter([100 + 100j], s=100, c="green", domain=Z_DOMAIN)
    ...
    def test_scatter_with_plot(self, smith_axes):
        ...
        smith_axes.plot([30 + 30j, 100 + 100j], "b-", domain=Z_DOMAIN)
        collection = smith_axes.scatter([30 + 30j, 50 + 50j, 100 + 100j], s=100, c="red", domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)

    def test_scatter_matched_load(self, smith_axes):
        """Test scatter at matched load (center of chart)."""
        collection = smith_axes.scatter([50 + 0j], s=200, c="green", marker="*", domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_numpy_array_input(self, smith_axes):
        ...
        collection = smith_axes.scatter(Z_array, s=100, domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)
    ...
    def test_scatter_real_only_input(self, smith_axes):
        ...
        x_vals = [25, 50, 75, 100]
        collection = smith_axes.scatter(x_vals, s=100, domain=Z_DOMAIN)
        assert isinstance(collection, PathCollection)
        assert len(collection.get_offsets()) == len(x_vals)

    def test_scatter_zorder(self, smith_axes):
        """Test that scatter respects zorder."""
        c1 = smith_axes.scatter([30 + 30j], s=100, domain=Z_DOMAIN)
        c2 = smith_axes.scatter([50 + 50j], s=100, domain=Z_DOMAIN)

        assert c2.get_zorder() > c1.get_zorder()

    @pytest.mark.parametrize("domain", [Z_DOMAIN, Y_DOMAIN, R_DOMAIN, NORM_Z_DOMAIN])
    def test_scatter_all_datatypes_parametrized(self, smith_axes, domain):
        """Parametrized test for all datatypes."""
        # R_DOMAIN with large impedance values will trigger warning
        if domain == R_DOMAIN:
            with pytest.warns(UserWarning, match="S-parameter magnitude"):
...
class TestScatterTransformations:
    ...
    def test_scatter_z_to_s_consistency(self, smith_axes):
        ...
        c1 = smith_axes.scatter([ZL], s=100, c="red", domain=Z_DOMAIN)
        c2 = smith_axes.scatter([Gamma], s=50, c="blue", domain=R_DOMAIN)
    ...
    def test_scatter_y_to_z_consistency(self, smith_axes):
        ...
        c1 = smith_axes.scatter([Z], s=100, c="red", domain=Z_DOMAIN)
        c2 = smith_axes.scatter([Y], s=50, c="blue", domain=Y_DOMAIN)
