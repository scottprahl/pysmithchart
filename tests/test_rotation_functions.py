...
class TestRotateByWavelength:
    ...
    def test_quarter_wave_transform(self):
        ...
        assert np.isclose(Z_rot, Z_expected, rtol=1e-5), \
            f"Quarter-wave: Expected {Z_expected}, got {Z_rot}"
    ...
    def test_half_wave_returns_to_start(self):
        ...
        assert np.isclose(Z_rot, Z, rtol=1e-5), \
            f"Half-wave should return to start: {Z} → {Z_rot}"
    ...
    def test_full_wave_returns_to_start(self):
        ...
        assert np.isclose(Z_rot, Z, rtol=1e-5), \
            f"Full wave should return to start: {Z} → {Z_rot}"
    ...
    def test_constant_vswr(self):
        ...
        for wavelength in [0.1, 0.25, 0.333, 0.5]:
            ...
            assert np.isclose(vswr_start, vswr_rot, rtol=1e-5), \
    ...
    def test_direction_toward_generator(self):
        ...
        assert not np.isclose(Z_toward_load, Z_toward_gen), \
        ...
        assert np.isclose(Z_back, Z, rtol=1e-5), \
            f"Should return to start: {Z} → {Z_toward_load} → {Z_back}"
    ...
    def test_matched_load_stays_matched(self):
        ...
        for wavelength in [0.1, 0.25, 0.5, 1.0]:
            Z_rot = utils.rotate_by_wavelength(Z, wavelength, Z0=Z0)
            assert np.isclose(Z_rot, Z, rtol=1e-5), \
                f"Matched load should stay matched: {Z} → {Z_rot} at λ={wavelength}"
...
class TestRotateTowardReal:
    ...
    def test_finds_target_resistance(self):
        ...
        assert np.isclose(Z_result.real, target_R, rtol=1e-5), \
            f"Real part should be {target_R}, got {Z_result.real}"
    ...
    def test_constant_vswr_maintained(self):
        ...
        assert np.isclose(vswr_start, vswr_result, rtol=1e-5), \
            f"VSWR should be preserved: {vswr_start} → {vswr_result}"
...
class TestRotateTowardImag:
    ...
    def test_finds_target_reactance(self):
        ...
        assert np.isclose(Z_result.imag, target_X, rtol=1e-4, atol=1e-6), \
    ...
    def test_rotate_to_real_axis(self):
        ...
        assert np.isclose(Z_result.imag, 0, atol=1e-6), \
            f"Should be on real axis, got Im(Z) = {Z_result.imag}"
    ...
    def test_two_solutions_exist(self):
        ...
        assert Z_high.real > Z_low.real, \
            f"higher_real ({Z_high.real}) should be > lower_real ({Z_low.real})"
...
class TestRotationFunctionsIntegration:
    ...
    def test_rotate_then_match_resistance(self):
        ...
        assert np.isclose(Z_matched.real, 50, rtol=1e-5), \
            f"Should have R=50Ω, got {Z_matched.real}Ω"
...
#             f"Should have X=0Ω, got {Z_full_matched.imag}Ω"
    ...
    def test_rotation_equivalence(self):
        ...
        assert np.isclose(abs(gamma_start), abs(gamma_rot), rtol=1e-5), \
            "VSWR should be preserved in rotation"
